...
<form id="yith-wcwl-form" action="<?php echo esc_url($yith_wcwl->get_wishlist_url()) ?>" method="post">
    ...
    <div id="ncm-wishtable" class="ncm-table">
        <table class="wishlist_table" cellspacing="0">
            <thead>
            <tr>
                <th class="product-remove header1"></th>
                <th class="product-thumbnail header2"></th>
                <th class="product-name header3">
                    <span class="rotated">&gt;</span>&nbsp;&nbsp;&nbsp;<span class="nobr"><?php _e('Product Name', 'yit') ?></span>
                </th>
                <?php if (get_option('yith_wcwl_price_show') == 'yes') : ?>
                    <th class="product-price header4">
                    <span class="rotated">&gt;</span>&nbsp;&nbsp;&nbsp;<span class="nobr"><?php _e('Unit Price', 'yit') ?></span>
                    </th><?php endif ?>
                <?php if (get_option('yith_wcwl_stock_show') == 'yes') : ?>
                    <th class="header5">
                    <span class="rotated">&gt;</span>&nbsp;&nbsp;&nbsp;<span class="nobr"><?php _e('Status', NCM_I18N_DOMAIN) ?></span>
                    </th><?php endif ?>
                <?php if (get_option('yith_wcwl_add_to_cart_show') == 'yes') : ?>
                    <th class="header6"><span class="nobr"></th><?php endif ?>
            </tr>
            </thead>
            <tbody>
            <?php
                    ...
                    if ($product_obj !== false && $product_obj->exists()) : ?>
                        <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                            <td class="product-remove">
                                <div>
                                    <a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url($yith_wcwl->get_remove_url($values['ID'])) ?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove withOpacityTransitionEffect" data-toggle="tooltip" title="<?php _e('Remove this product', 'yit') ?>"></a>
                            </td>
                            <td class="product-thumbnail">
                                <a href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $values['prod_id']))) ?>">
                                    <?php echo str_replace("<img", "<img class='patternBorderColorHover'", $product_obj->get_image()); ?>
                                </a>
                            </td>
                            <td class="product-name">
                                <a class="patternTextColor" href="<?php echo esc_url(get_permalink(apply_filters('woocommerce_in_cart_product', $values['prod_id']))) ?>"><?php echo apply_filters('woocommerce_in_cartproduct_obj_title', $product_obj->get_title(), $product_obj) ?></a>
                            </td>
                            <?php if (get_option('yith_wcwl_price_show') == 'yes') : ?>
