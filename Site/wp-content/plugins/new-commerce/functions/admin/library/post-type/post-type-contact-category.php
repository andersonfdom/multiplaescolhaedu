<?php 
	/* POST TYPE - CONTACT CATEGORY */

	// register the contact category post type
	function ncm_register_post_type_contact_category() {
		$labels = array(
			"name" => __("Contact Categories", NCM_I18N_DOMAIN),
			...
			"parent_item_colon" => "",
			"menu_name" => __("Departments", NCM_I18N_DOMAIN)
	  	);
	  	$args = array(
			"labels" => $labels,
			"public" => false,
			"show_ui" => true,
			"rewrite" => array("slug" => "ncm_contact_category"),
    	...
    	register_post_type("ncm_contact_category", $args);
	}
